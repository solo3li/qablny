...
function LocalVideo({ isCameraOn }: { isCameraOn: boolean }) {
  const tracks = useTracks([Track.Source.Camera]);
  const localTrack = tracks.find((t: any) => t.participant.isLocal === true) as TrackReference | undefined;
...
export default function MatchScreen() {
  ...
  const handleStartSearch = async () => {
    ...
    try {
      ...
      setIsSearching(false);
      Alert.alert('يجب تسجيل الدخول أولاً لتتمكن من استخدام ميزة المطابقة.');
    }
  };
  ...
  const handleAddFriend = async () => {
    try {
      ...
      Alert.alert('تم إرسال طلب الصداقة بنجاح!');
    } catch (e: any) {
              ...
              {gifts.map((g) => (
                <TouchableOpacity key={g.id} style={styles.giftItem} onPress={() => handleSendGift(g)}>
                  <View style={styles.giftEmojiBox}>
                    {g.gifUrl ? (
                      <Image source={{ uri: g.gifUrl }} style={{ width: 40, height: 40 }} contentFit="contain" />
                    ) : (
...
const styles = StyleSheet.create({
  ...
  // PIP Local Video
  pipContainer: { position: 'absolute', top: Platform.OS === 'ios' ? 120 : 100, right: 20, width: 100, height: 150, borderRadius: 20, overflow: 'hidden', borderWidth: 2, borderColor: '#fff', backgroundColor: '#000', zIndex: 30, shadowColor: '#000', shadowOffset: {width: 0, height: 8}, shadowOpacity: 0.4, shadowRadius: 12 },
  ...
  // Home screen top bar
  homeTopBar: { position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: Platform.OS === 'ios' ? 56 : 48, paddingHorizontal: 20, zIndex: 20 },
  ...
  // Filters UI
  filterIconBtn: { position: 'absolute', top: Platform.OS === 'ios' ? 116 : 108, right: 20, width: 44, height: 44, borderRadius: 22, backgroundColor: Colors.primaryDim, borderWidth: 1, borderColor: Colors.primary, alignItems: 'center', justifyContent: 'center', zIndex: 10 },
